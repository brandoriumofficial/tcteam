...
export default function AdminCouponOffer() {
  ...
  const [offers, setOffers] = useState([
    ...
    { 
      id: 4, 
      name: "Summer Body Care", 
      desc: "15% OFF on all body care products", 
      status: "Active", 
      type: "Flat Discount",
      discount: "15%",
      category: "Body Care",
      products: ["Body Wash", "Body Lotion", "Deodorant"],
      startDate: "2024-03-01",
      endDate: "2024-06-30",
      bannerImage: "https://via.placeholder.com/400x100/06b6d4/ffffff?text=Summer+Special",
      tags: ["Summer", "Body Care", "Limited"],
      priority: 4,
  ...
  useEffect(() => {
    ...
    const updatedOffers = offers.map(offer => {
      ...
      // If expired
      if (end < today && offer.status !== "Expired") {
        return { ...offer, status: "Expired" };
      }
      
      // If not yet started
      if (start > today && offer.status === "Active") {
        return { ...offer, status: "Upcoming" };
      }
      
      // If currently active
      if (start <= today && end >= today && offer.status !== "Active") {
  ...
  const saveCoupon = () => {
    ...
    const newCoupon = {
      ...
      createdAt: couponForm.id ? couponForm.createdAt : new Date().toISOString().split('T')[0]
    };
  ...
  const saveOffer = () => {
    ...
    const newOffer = {
      ...
      createdAt: offerForm.id ? offerForm.createdAt : new Date().toISOString().split('T')[0]
    };
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Manage all discounts and promotional offers</p>
          </div>
        ...
        <div className="space-y-6">
              ...
              <div className="flex justify-between items-center pt-4 border-t">
                ...
                <div className="flex gap-3">
                  <button 
                    ...
                    className="px-6 py-2 border border-gray-300 text-gray-600 rounded-lg font-bold text-sm hover:bg-gray-50"
                  >
                    Clear
                  </button>
                  <button 
          ...
          <div className={`rounded-xl shadow-sm border overflow-hidden ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            ...
            <div className={`px-6 py-4 border-b flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 ${darkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-gray-50'}`}>
              <h2 className="font-bold text-lg">All Coupons ({filteredCoupons.length})</h2>
      ...
      {activeTab === "offers" && (
        <div className="space-y-6">
          ...
          <div className={`rounded-xl shadow-lg border border-t-4 border-t-purple-500 p-6 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-bold text-lg flex items-center gap-2">
                ...
                {offerForm.id ? "✏️ Edit Offer" : "➕ Create New Offer"}
              </h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase">Discount Details</label>
                <input 
                  name="discount"
              ...
              <div className="flex gap-3 pt-2">
                <button 
                  ...
                  className="flex-1 border border-gray-300 text-gray-600 py-2 rounded-lg font-bold text-sm hover:bg-gray-50"
                >
                  Clear
                </button>
                <button 
          ...
          <div className={`rounded-xl shadow-sm border overflow-hidden ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            {/* Filter & Sort Bar */}
            <div className={`px-6 py-4 border-b flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 ${darkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-gray-50'}`}>
              <h2 className="font-bold text-lg">All Offers ({filteredOffers.length})</h2>
              ...
              <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                ...
                <div className="flex gap-2">
                  <select 
                    value={offerFilter.category}
                    onChange={(e) => setOfferFilter({...offerFilter, category: e.target.value})}
                    className={`px-3 py-2 rounded-lg text-sm ${darkMode ? 'bg-gray-700 border-gray-600' : 'border-gray-300'}`}
                  >
                    <option value="All">All Categories</option>
                    <option value="Hair Care">Hair Care</option>
                  ...
                  <select 
                    ...
                    onChange={(e) => setOfferFilter({...offerFilter, status: e.target.value})}
                    className={`px-3 py-2 rounded-lg text-sm ${darkMode ? 'bg-gray-700 border-gray-600' : 'border-gray-300'}`}
                  >
                    <option value="All">All Status</option>
                    <option value="Active">Active</option>
                  ...
                  <select 
                    value={offerFilter.sortBy}
                    onChange={(e) => setOfferFilter({...offerFilter, sortBy: e.target.value})}
                    className={`px-3 py-2 rounded-lg text-sm ${darkMode ? 'bg-gray-700 border-gray-600' : 'border-gray-300'}`}
                  >
                ...
                {filteredOffers.map(offer => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="text-sm font-bold text-purple-600">{offer.discount}</div>
                      <div className="text-xs text-gray-500">
                        {formatDateDisplay(offer.startDate)} - {formatDateDisplay(offer.endDate)}
                      </div>
                    </div>
      ...
      {activeTab === "products" && (
        <div className="space-y-6">
          ...
          <div className={`rounded-xl shadow-sm border p-6 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className="flex justify-between items-center mb-6">
              <h2 className="font-bold text-lg">Product-wise Offers</h2>
              <div className="flex gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {productOffers.map(product => (
                <div 
                  key={product.id} 
                  className={`p-4 rounded-xl border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} hover:shadow-md transition-shadow`}
                >
                  <div className="flex items-center gap-3">
                  ...
                  <div className="mt-4 flex justify-between items-center">
                    <button className="text-xs px-3 py-1 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100">
                      View Details
                    </button>
                    <div className="text-xs text-gray-500">
                      Applied to: 125 orders
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="mt-6 pt-6 border-t border-gray-200">
              <button className="w-full py-3 border-2 border-dashed border-gray-300 rounded-xl text-gray-500 hover:border-green-500 hover:text-green-600 transition-colors flex items-center justify-center gap-2">
                <FaPlus />
          ...
          {/* Performance Stats */}
          <div className={`rounded-xl shadow-sm border p-6 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <h2 className="font-bold text-lg mb-4">Offer Performance</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
                <div className="text-sm text-gray-500">Most Popular Offer</div>
                <div className="font-bold text-lg mt-1">Diwali Festival Sale</div>
                <div className="text-xs text-green-600">Applied 542 times</div>
              </div>
              <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
                <div className="text-sm text-gray-500">Highest Discount Usage</div>
                <div className="font-bold text-lg mt-1">WELCOME25</div>
                <div className="text-xs text-blue-600">120/200 used</div>
              </div>
              <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
                <div className="text-sm text-gray-500">Upcoming Expiry</div>
                <div className="font-bold text-lg mt-1">3 coupons</div>
